$(document).ready(function(){
	...
	$("#btnDangNhap").click(function(){
		...
		$.ajax({
			url:"/detaiwww/api/KiemTraDangNhap",
		    type:"GET",
		    data:{
		    	email:ten,
		    	matkhau:password
...
//		}).done(function(){
//			$.ajax({
//				url:"/detaiwww/api/LaySoLuongGioHang",
//			    type:"GET",
//				success: function(value){
//					$("#giohang").find("div").addClass("circle-giohang");
//					$("#giohang").find("div").html("<span>" + value + "</span>") ;
...
//		});
	...
	$(".soluong-giohang").change(function(){
		...
		$.ajax({
			url:"/detaiwww/api/CapNhatGioHang",
		    type:"GET",
		    data:{
		    	masp:masp,
		    	masize:masize,
...
//	})
	...
	$("body").on("click",".paging-item",function(){
		...
		$.ajax({
			url:"/detaiwww/api/LaySanPhamLimit",
		    type:"GET",
		    data:{
		    	spbatdau:spbatdau,
	...
	$("#hinhanh").change(function(event){
		...
		$.ajax({
			url:"/detaiwww/api/UploadFile",
		    type:"POST",
		    data:forms,
		    contentType:false,
		    processData:false,//khong xu ly form data
	...
	$("#btnThemSanPham").click(function(event){
		...
		arraychitiet = [];
		 ...
		 });//dinh dang lai theo dang a:fdf
		
		 $("#containerchitietsanpham >.chitietsanpham").each(function(){
			 objectChitiet = {};
			 mausanpham = $(this).find('select[name="mausanpham"]').val();//********** mau 
	...
	$("#btnCapNhatSanPham").click(function(event){
		...
		arraychitiet = [];
		 ...
		 $.ajax({
				url:"/detaiwww/api/capnhatsanpham",
			    type:"POST",
			    data:{
			    	dataJson: JSON.stringify(json)
			    },
	...
	$("body").on("click",".capnhatsanpham",function(){
		...
		$.ajax({
			url:"/detaiwww/api/LayDanhSachSanPhamTheoMa",
		    type:"POST",
		    data:{
		    	masanpham: masanpham
		    },
...
//	<td class="tinhtrang">
//								 <select name="tinhtrang" class="form-control" id="tinhtranghanghoa">
//								<option value="${dh.getTinhtrang()}">${dh.getTinhtrang()}</option>
...
//							<td class="ghichu">${dh.getGhichu() }</td>
	$(".btn_duyet").click(function(){
		//var self = $(this);
		var madonhang = $(this).closest("tr").find(".madonhang").text();
		var ten = $(this).closest("tr").find(".ten").text();
		var sdt = $(this).closest("tr").find(".sdt").text();
		var diachigiaohang = $(this).closest("tr").find(".diachigiaohang").text();
		
		
		var tinhtrang = $(this).closest("tr").find("#tinhtranghanghoa").val();
		// var tinhtrang = $(this).closest("tr").find('select[name="tinhtrang"]').text();
		
		var ngaylap = $(this).closest("tr").find(".ngaylap").text();
		var hinhthucgiaohang = $(this).closest("tr").find(".hinhthucgiaohang").text();
		var ghichu = $(this).closest("tr").find(".ghichu").text();
		alert(ten+tinhtrang+ngaylap+hinhthucgiaohang+ghichu);
		$.ajax({
			url:"/detaiwww/api/xulydonhang",
		    type:"GET",
		    data:{
		    	mahoadon:madonhang,
		    	tenkhachhang:ten,
